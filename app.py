...
class SpectroApp(tk.Tk): #
    ...
    def _build_live_tab(self): #
        # This function would contain the UI build code for the Live View tab.
        # It's kept separate for clarity, potentially in 'tabs/live_view_tab.py'.
        # For this example, we'll assume it sets up self.live_ax, self.live_line, etc.
        # from tabs.live_view_tab import build as _build #
        # _build(self) #
        pass # Placeholder

    def _build_measure_tab(self): #
        # Similar placeholder for Measurement Tab UI build.
        # from tabs.measurements_tab import build as _build #
        # _build(self) #
        pass # Placeholder

    def _build_analysis_tab(self): #
        # Similar placeholder for Analysis Tab UI build.
        # from tabs.analysis_tab import build as _build #
        # _build(self) #
        pass # Placeholder

    def _build_setup_tab(self): #
        # Similar placeholder for Setup Tab UI build.
    ...
    def connect_spectrometer(self): #
        ...
        try:
            ...
            # ava.nav = 1 # Property 'nav' not found in Avantes_Spectrometer
            ava.ncy_requested = 1 # Property 'ncy' not found, using ncy_requested
    ...
    def _live_loop(self): #
        ...
        while self.live_running.is_set(): #
            ...
            try:
                ...
                # The 'elif not np.all(np.isfinite(y)):' block was removed here
                # as requested, allowing _update_live_plot to handle NaNs/Infs.
                # === SECTION REMOVED END ===

                # 6. Schedule plot update ONLY if data is valid (or if validation removed)
                if data_valid: # Check remains, but condition is less strict #
            ...
            if result: #
                ...
                try:
                    if hasattr(self, 'lasers'): #
                        self.lasers.close_all() # Method 'close_all' not found in LaserController #
