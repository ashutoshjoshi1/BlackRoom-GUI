...
laser_power = {"405": 0.005, "445": 0.003, "488": 0.03}
laser_power_640nm = 0.03
#all_lasers = ["532", "517", "488", "445", "405", "377", "Hg_Ar"]
all_lasers = ["532", "445", "405", "377", "Hg_Ar"]
#all_lasers = ["405", "Hg_Ar"]
#all_lasers = ["Hg_Ar"]
data_rows = []
...
#ax.set_yscale('log')
ax.set_title(f"Spectrometer= {ava.sn}: Live Measurement", fontsize=14)
...
try:
    ...
    for lwl in all_lasers:
        ...
        while True:
            ...
            line.set_ydata(y)
            ax.set_title(f"Spectrometer= {ava.sn}: Live Measurement for {lwl} nm | IT = {it_ms:.1f} ms | peak={peak:.0f}")
            fig.canvas.draw()
            fig.canvas.flush_events()
        ...
        if TARGET_LOW <= peak <= TARGET_HIGH:
            ...
            ax.set_title(f"Spectrometer= {ava.sn}: {lwl} nm DARK Measurement @ IT = {it_ms:.1f} ms")
            fig.canvas.draw()
            fig.canvas.flush_events()
    ...
    fig640, ax640 = plt.subplots(figsize=(12, 6))
    ax640.set_title(f"Spectrometer= {ava.sn}: Live 640 nm OBIS Measurement", fontsize=14)
    ax640.set_xlabel("Pixel Index")
    ax640.set_ylabel("Counts")
...
plt.title(f"Spectrometer= {ava.sn}: Normalized LSFs")
...
plt.title(f"Spectrometer= {ava.sn}: Dark-Corrected 640 nm Measurements")
plt.xlabel("Pixel Index")
...
peaks, props = find_peaks(signal_corr, prominence=0.014*np.max(signal_corr), distance=20)
...
plt.title(f"Spectrometer= {ava.sn}: Hg-Ar Lamp Spectrum with Detected Peaks", fontsize=16)
plt.legend()
plt.grid(True)
plt.tight_layout()
...
plt.title(f"Spectrometer= {ava.sn}: Spectral Distribution Function (SDF)",fontsize=16)
#plt.ylim(0,0.0035)
...
plt.title(f'Spectrometer= {ava.sn}: Spectral Resolution vs Wavelength', fontsize=16)
plt.grid(True)
plt.legend()
plt.tight_layout()
...
def compute_width_at_percent_max(x, y, percent=0.2):
    """
    Compute the full width at a given percent (e.g., 20%) of max height.
    Parameters:
        x : np.ndarray
            X-axis values (wavelength or pixels).
        y : np.ndarray
            Y-axis values (LSF amplitude).
        percent : float
            Percentage of the max height at which to calculate the width (0 < percent < 1).
    Returns:
        width : float
            Width at the specified percent of maximum height.
    """
    y = y - np.min(y)        # Remove baseline
    ...
    if len(above) < 2:
        return 0.0  # Not enough points above threshold
...
plt.title(f"Spectrometer= {ava.sn}: Slit Function with FWHM")
...
for lsf, peak_pixel, λ0 in zip(lsf_list, pixel_locations, laser_wavelengths):
    ...
    fw_20 = compute_width_at_percent_max(x, lsf_norm, percent=0.2)
    ax1.plot(x, lsf_norm, label=f'{λ0} nm, FWHM = {fwhm:.2f} nm, FW_20p = {fw_20:.2f} nm')
...
for λ0 in pandora_2_wavelengths:
    ...
    if not df_w.empty:
        ...
        fw_20 = compute_width_at_percent_max(x_ref, y_ref, percent=0.2)
        ax1.plot(x_ref, y_ref, '--', linewidth=2, label=f'Pan 2, {λ0} nm, FWHM = {fwhm:.2f} nm, FW_20p = {fw_20:.2f} nm')
...
for λ0 in pandora_334_wavelengths:
    ...
    if not df_334.empty:
        ...
        fw_20 = compute_width_at_percent_max(x_334, y_334, percent=0.2)
        ax1.plot(x_334, y_334, '--', linewidth=2, label=f'Pan 334, {λ0} nm, FWHM = {fwhm_:.2f} nm, FW_20p = {fw_20:.2f} nm', color = 'black')

ax1.set_yscale('log')
ax1.set_title(f"Spectrometer = {ava.sn}: Normalized LSFs of Lasers", fontsize=14)
...
ax1.grid(True)
ax1.legend(title="Laser Wavelength", fontsize=12)
...
for lsf, peak_pixel, λ0 in zip(lsf_list_lamp, matched_pixels, matched_wavelengths):
    ...
    fw_20 = compute_width_at_percent_max(x, lsf_norm, percent=0.2)
    ax2.plot(x, lsf_norm, label=f'{λ0} nm, FWHM = {fwhm:.2f} nm, FW_20p = {fw_20:.2f} nm')
...
ax2.grid(True)
ax2.legend(title="Lamp Wavelength", fontsize=12)
